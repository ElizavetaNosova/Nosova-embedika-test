...
class KremlinGenerator:
    ...
    def generate(
        self, person="", organization="", forse_person=False, num_return_sequences=None
    ) -> List[Tuple[str, str]]:
        ...
        gpt_output = self.gpt.generate(**encoding, **generation_parameters)
        decoded_output = [self.tokenizer.decode(i) for i in gpt_output.tolist()]
        return [get_title_and_text(i) for i in decoded_output]

    def _format_prompt(self, person: str, organization: str, forse_person: bool):
