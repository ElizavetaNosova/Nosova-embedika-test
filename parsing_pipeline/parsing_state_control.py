...
class KremlinParsingFileStateManager(ParsingFileStateManager):
    def __init__(
        ...
        self.temp_child_directory_path = os.path.join(
            self.data_directory, temp_child_directory
        )
        # логи истории посещений страниц меню
        self.page_log_file_path = os.path.join(
            self.temp_child_directory_path, page_log_filename
        )
        ...
        self.links_collection_output_file_path = os.path.join(
            self.temp_child_directory_path, links_collection_output_filename
        )
        filepath_without_extention, extention = os.path.splitext(
            self.links_collection_output_file_path
        )
        # файл, куда записываются ссылки в процессе сбора
        self.temp_links_collection_output_file_path = (
            filepath_without_extention + "_temp" + extention
        )
    ...
    def get_all_processed_links(self) -> List[str]:
        ...
        # считываем заново
        if (
            not hasattr(self, "__read_file_modification_time")
    ...
    def get_scrapped_links(self):
        ...
        with open(self.links_collection_output_file_path) as f:
            return [
                ...
                if i.strip() and i.strip() != self.end_scrapping_log
            ]

    def new_links_exist(self) -> bool:
        known_links = self.get_all_processed_links()
        scrapper = KremlinScrapper(
