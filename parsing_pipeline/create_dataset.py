...
def json_files_to_dataset(json_files_directory, obligatory_keys, columns_order):
    ...
    for filename in os.listdir(json_files_directory):
        ...
        if os.path.isfile(filepath) and filename.endswith(".json"):
            ...
            if isinstance(sample, dict) and all(
                [sample.get(key) for key in obligatory_keys]
            ):
...
def create_ner_dataset(
    ...
    df = json_files_to_dataset(
        json_files_directory, obligatory_keys=[], columns_order=columns_order
    )
