...
class CollectKremlinUrlsTask(luigi.Task):
    ...
    def run(self):
        try:
            known_links = self.parsing_state_manager.get_all_processed_links() if self.update_mode else set()
        ...
        shutil.copy(self.parsing_state_manager.temp_links_collection_output_file_path,
                    self.parsing_state_manager.links_collection_output_file_path)


    def output(self):
        return luigi.LocalTarget(self.parsing_state_manager.links_collection_output_file_path)

    def requires(self):
        return [MakeDirectoryTask(os.path.dirname(self.parsing_state_manager.temp_child_directory_path))]
...
class AllArticlesAreParsed(luigi.Target):
    ...
    def exists(self):
        if not os.path.exists(self.parsing_state_manager.links_collection_output_file_path):
            return False
...
class CollectDataTask(luigi.Task):
    ...
    def run(self):
        ...
        yield [ParseKremlinArticleTask(content_url=url,
                    output_file_path=self.parsing_state_manager.get_path_to_save_parsing_output(url)
                                       ) for url in urls]

    def requires(self):
        return CollectKremlinUrlsTask(parsing_state_manager=self.parsing_state_manager,
                                      update_mode=self.update_mode)
...
class CreateOrUpdateNerDatasetTask(luigi.Task):
    ...
    def requires(self):
        return [MakeDirectoryTask(self.parsing_state_manager.data_directory),
                MakeDirectoryTask(self.parsing_state_manager.temp_child_directory_path),
                CollectDataTask(parsing_state_manager=self.parsing_state_manager,
                                update_mode=self.update_mode)]